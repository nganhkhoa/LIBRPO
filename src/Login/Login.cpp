...
int AccountLoginPrompt(vector<User>& UserData, int& User_num) {
	...
	ShowAccountList(UserData, User_num);//show the account with no pass and active state

	cout << "Lua chon cua ban? "; int Choice; cin >> Choice;
	...
	while (Choice > UserData[User_num].AccountList.size() || Choice < 0) {
		cout << "Tai khoan so " << Choice << " khong co, moi nhap lai: " ; cin >> Choice;
	}
...
bool RightPassword(vector<User>& UserData, UserAccount recordNum, int& Attemps) {
	system("cls");

	string AccountID = UserData[recordNum.User_num].AccountList[recordNum.Account_num].AccId;
	...
	if (Attemps == 0) cin.ignore();//first attemp
	else if (Attemps < MAX_ATTEMPT) {//wrong but still ok
		cout << "Ban con " << MAX_ATTEMPT - Attemps << " lan thu" << endl;
		cout << "Neu ban nhap sai qua " << MAX_ATTEMPT << " lan, tai khoan se bi khoa" << endl;
	} else if (Attemps == MAX_ATTEMPT) {///lock this user
		LockAccount(AccountID);
		cout << "Tai khoan nay da bi khoa do nhap sai mat khau nhieu lan" << endl;
		cout << "Hay lien he ban quan tri de lay lai quyen truy cap tai khoan nay" << endl;
		system("pause");
	...
	string ThePassword = UserData[recordNum.User_num].AccountList[recordNum.Account_num].Password;
...
LoggedInUser CreateLoggedInUser(vector<User>& UserData, UserAccount recordNum) {
	...
	NewUser.AccId =	UserData[recordNum.User_num].AccountList[recordNum.Account_num].AccId;
	...
	for (int Role_num = 0; 
		...
		Role_num++) {//to long for ya heh
			NewUser.RoleId.push_back(
				UserData[recordNum.User_num].AccountList[recordNum.Account_num].RoleId[Role_num]
				);
	}
	return NewUser;
}
...
LoggedInUser Login(vector<User>& UserData) {
	...
	while (true) {
		if (UserLoginPrompt(UserData, User_num)) {
			...
			if (User_num == UserData.size()) {
				...
				system("pause");
				continue;
			} else break;//if found proceed account choosing
		} else { //if User_num = -1
			cout << "\nBan khong muon dang nhap nua? (y/n) "; 
			string str; getline(cin, str);
			if (str == "y") {
				return NewActiveUser; //false
			} else continue; //login prompt
		}
	}

	int Account_num = AccountLoginPrompt(UserData, User_num) - 1;//chose account
	if (Account_num == -1) return NewActiveUser; //no account choose
	UserAccount recordNum = {User_num, Account_num}; //save user place in vector
	...
	if (Attemps == 0) NewActiveUser = Login(UserData);
		...
		//attemps set to 0
	else NewActiveUser = CreateLoggedInUser(UserData, recordNum);
