...
int ChooseUtilities(LoggedInUser CurrentUser) {
	...
	int role;
	cout << "Ban chon thuc hien chuc nang nao? "; cin >> role;

	while (role < 0 || role > CurrentUser.RoleId.size()) {
		cout << "Chuc nang khong co, moi chon lai: "; cin >> role;
	}
	return role;
...
void Utilities(LoggedInUser CurrentUser) {
	while (true) {
		...
		enum Roles {DocGiaRole = 1, ThuThuRole, QLNDRole, QLTCRole};
		switch (CurrentUser.RoleId[role - 1])
