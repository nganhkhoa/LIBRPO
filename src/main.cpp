...
int main() {
	...
	while (true) {//also run welcome until exit
		while (!CurrentUser.Active) {//if not active, show general welcom
			enum WelcomeChoice {LoginUser = 1, SignUpUser, BrowseOption, Manual, Exit};
			...
			switch(Welcome()) {
				case LoginUser:
					...
					if (CurrentUser.Active) LoginHistory(CurrentUser); //login history
					break;
				case SignUpUser:
					//SignUp();
					break;
				...
				case Exit: {//normal exit
					cout << "Ban muon thoat? (y/n) "; cin.ignore();
					string Answer; getline(cin, Answer);
					if (Answer != "y") break;
					else return 0; //terminate the program
				}
				default:
					break;
			}
		...
		while (CurrentUser.Active) {
			enum WelcomeUserChoice {LogoutUser = 1, SettingUser, UtilitiesUser, BrowseOption, Manual, Exit};
			switch(WelcomeUser(CurrentUser)) {
				case LogoutUser: {//as said change active to false
					cout << "Ban muon dang xuat? (y/n) "; cin.ignore();
					string Answer; getline(cin, Answer);
					if (Answer == "y") {
