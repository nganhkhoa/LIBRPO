...
bool ReadBookFromText(BookManage& LIBPROBook)
{
    ...
    if(!content.is_open()) {std::cout<<"Cannot open file.Exitting..."; return false;}
    ...
    while (!content.eof())
    {
        int bracket=0; bool SingleSpace=false;
        ...
        while (step<length)
        {
            if(step<13)
            {
                Contemp.ISBN+=input[step]; step++;
                if (step==13) {step++;}
                continue;
            }
            if (input[step]=='|') {step++; continue;}
            if (input[step]==' '    &&      SingleSpace==false  &&      bracket==0)
            {
                SingleSpace=true;
                //td::cout<<singleWord<<","<<step<<":"<<NumOfTitle<<"|";
            ...
            if (input[step]==' '&&SingleSpace==true) {step++; continue;}
            if (input[step]==',') {step++; continue;}
            if (input[step]=='.') {step++; continue;}
            if (input[step]=='?') {step++; continue;}
            if (input[step]=='!') {step++; continue;}
            if (input[step]==':') {step++; continue;}
            if (input[step]=='[') {bracket=1; step+=2; continue;}
            if (input[step]==']')
            {
                Contemp.FindWord[NumOfWord]=singleWord;
        ...
        NumOfBook++;NumOfWord=0; NumOfTitle=-1;
        step=0;
    }
    content.close();

    //for (int check=0; check< 150; check++ )std::cout<<check<<LIBPROBook.ResourceContent[1].FindWord[check]<<std::endl;
...
void ReadWordsToFind(std::string* search)
{
    ...
    while(core[step]!='\0')
    {
        ...
        else if (core[step]==' '&&starting==false) {NumOfWord++; step++;}
        else
        {
...
void CompareWords(std::string search[], BookManage& LIBPROBook, std::vector<std::string> result)
{
    ...
    for (int BookPlace=0; BookPlace<LIBPROBook.BookPosition.size(); BookPlace++)
    {
        for (int step=0; step<SEARCH_MAX; step++)
        {
            while (Position++<TITLE_MAX-1&&FirstWordFound==0)
            {
                if (search[step]==LIBPROBook.ResourceContent[BookPlace].Title[Position])
                {
                    result.push_back(LIBPROBook.ResourceContent[BookPlace].ISBN);
                    FirstWordFound=1; break;
                }
            }

            while (ContentPos++<CONTENT_MAX-1&&FirstWordFound==0)
            {
                if (search[step]==LIBPROBook.ResourceContent[BookPlace].FindWord[ContentPos])
                {
                    result.push_back(LIBPROBook.ResourceContent[BookPlace].ISBN);
                    FirstWordFound=1; break;
                }
    ...
    for (int showBookFound=0; showBookFound< result.size(); showBookFound++)
    {
        for (int isbn=0; isbn<LIBPROBook.BookPosition.size(); isbn++)
        {
            if (LIBPROBook.ResourceContent[isbn].ISBN==result[showBookFound])
            {
                std::cout<<numberOfFound<<". ";
                while(titleValue++<TITLE_MAX-1) std::cout<<LIBPROBook.ResourceContent[isbn].Title[titleValue]<<" ";
                hasShown=true;
            }
            if (hasShown==true) {std::cout<<std::endl; hasShown=false;}
        }
        titleValue=-1;
        numberOfFound++;
    }
